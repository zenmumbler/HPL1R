    ...
    dgTriplex& normalAB, dgInt32 threadIndex) const
{
  ...
  else if (collisionA->IsType(dgCollision::dgConvexCollision_RTTI)
      && collisionB->IsType(dgCollision::dgConvexCollision_RTTI))
  {
    dgInt32 flag;
...
//		contactMaterial.m_overrideNormalAccel = false;
    contactMaterial.m_flags = dgContactMaterial::m_collisionEnable__
        | (material->m_flags
            & (dgContactMaterial::m_friction0Enable__
...
//		contactMaterial.m_overrideNormalAccel = false;
    ...
    if (staticMotion)
    {
      ...
      else
      {
        ...
        contactMaterial->m_dir1 = contactMaterial->m_normal
            * contactMaterial->m_dir0;
        _ASSERTE(
  ...
  if (err2 < (DG_CACHE_DIST_TOL * DG_CACHE_DIST_TOL))
  {
    ...
    if (err2 < (DG_CACHE_DIST_TOL * DG_CACHE_DIST_TOL))
    {
      ...
      if (err2 < (DG_CACHE_DIST_TOL * DG_CACHE_DIST_TOL))
      {
        ...
        {
          dgMatrix matrix0(
              dgMatrix(contact->m_prevRotation0, contact->m_prevPosit0).Inverse()
                  * body0->m_matrix);
          dgMatrix matrix1(
              dgMatrix(contact->m_prevRotation1, contact->m_prevPosit1).Inverse()
                  * body1->m_matrix);
    ...
    dgCollisionParamProxy& proxy) const
{
  ...
  contactCount =
      ((dgCollisionCompound *) compoundBody->m_collision)->CalculateContacts(
          pair, proxy, 1);
    ...
    dgCollisionParamProxy& proxy) const
{
  ...
  if (sceneProxy.m_shape->IsType(dgCollision::dgConvexCollision_RTTI))
  {
    ...
    proxy.m_maxContacts =
        ((DG_MAX_CONTATCS - pair->m_contactCount) > 16) ?
            16 : DG_MAX_CONTATCS - pair->m_contactCount;

    proxy.m_contacts = &pair->m_contactBuffer[pair->m_contactCount];
    pair->m_contactCount = pair->m_contactCount
        + dgInt16(CalculateConvexToConvexContactsSimd(proxy));
...
//		pair->m_isTrigger = proxy.m_inTriggerVolume;
  ...
  else
  {
    ...
    proxy.m_floatingMatrix = sceneProxy.m_matrix;
    proxy.m_maxContacts =
        ((DG_MAX_CONTATCS - pair->m_contactCount) > 32) ?
            32 : DG_MAX_CONTATCS - pair->m_contactCount;

    proxy.m_contacts = &pair->m_contactBuffer[pair->m_contactCount];
    pair->m_contactCount = pair->m_contactCount
        + dgInt16(CalculateConvexToNonConvexContactsSimd(proxy));
    if (pair->m_contactCount
        > (DG_MAX_CONTATCS - 2 * (DG_CONSTRAINT_MAX_ROWS / 3)))
    {
      pair->m_contactCount = dgInt16(
    ...
    dgCollisionParamProxy& proxy) const
{
  ...
  if (sceneProxy.m_shape->IsType(dgCollision::dgConvexCollision_RTTI))
  {
    ...
    proxy.m_contacts = &pair->m_contactBuffer[pair->m_contactCount];
    pair->m_contactCount = pair->m_contactCount
        + dgInt16(CalculateConvexToConvexContacts(proxy));
    if (pair->m_contactCount
        > (DG_MAX_CONTATCS - 2 * (DG_CONSTRAINT_MAX_ROWS / 3)))
    {
      pair->m_contactCount = dgInt16(
...
//		pair->m_isTrigger = proxy.m_inTriggerVolume;
  ...
  else
  {
    ...
    proxy.m_contacts = &pair->m_contactBuffer[pair->m_contactCount];
    pair->m_contactCount = pair->m_contactCount
        + dgInt16(CalculateConvexToNonConvexContacts(proxy));
    if (pair->m_contactCount
        > (DG_MAX_CONTATCS - 2 * (DG_CONSTRAINT_MAX_ROWS / 3)))
    {
      pair->m_contactCount = dgInt16(
...
//	proxy.m_continueCollision = material->m_collisionContinueCollisionEnable & (body0->m_continueCollisionMode | body1->m_continueCollisionMode);
  proxy.m_continueCollision = dgInt32(
      ((material->m_flags
          & dgContactMaterial::m_collisionContinueCollisionEnable__) ? 1 : 0)
          & (body0->m_continueCollisionMode | body1->m_continueCollisionMode));
  proxy.m_isTriggerVolume = body0->m_collision->IsTriggerVolume()
      | body1->m_collision->IsTriggerVolume();
