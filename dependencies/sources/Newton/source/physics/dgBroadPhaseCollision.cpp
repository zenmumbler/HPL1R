...
dgBroadPhaseCollision::dgBroadPhaseCollision(dgMemoryAllocator* allocator) :
    m_min(-dgFloat32(1000.0f), -dgFloat32(1000.0f), -dgFloat32(1000.0f),
        dgFloat32(0.0f)), m_max(dgFloat32(1000.0f), dgFloat32(1000.0f),
        dgFloat32(1000.0f), dgFloat32(0.0f)), m_appMinBox(-dgFloat32(1000.0f),
        -dgFloat32(1000.0f), -dgFloat32(1000.0f), dgFloat32(0.0f)), m_appMaxBox(
    ...
    const dgVector& max)
{
  ...
  m_min.m_x = dgFloor(m_min.m_x / dgFloat32(1 << DG_OCTREE_MAX_DEPTH))
      * dgFloat32(1 << DG_OCTREE_MAX_DEPTH);
  m_min.m_y = dgFloor(m_min.m_y / dgFloat32(1 << DG_OCTREE_MAX_DEPTH))
      * dgFloat32(1 << DG_OCTREE_MAX_DEPTH);
  m_min.m_z = dgFloor(m_min.m_z / dgFloat32(1 << DG_OCTREE_MAX_DEPTH))
      * dgFloat32(1 << DG_OCTREE_MAX_DEPTH);

  m_max.m_x = dgCeil(m_max.m_x / dgFloat32(1 << DG_OCTREE_MAX_DEPTH))
      * dgFloat32(1 << DG_OCTREE_MAX_DEPTH);
  m_max.m_y = dgCeil(m_max.m_y / dgFloat32(1 << DG_OCTREE_MAX_DEPTH))
      * dgFloat32(1 << DG_OCTREE_MAX_DEPTH);
  m_max.m_z = dgCeil(m_max.m_z / dgFloat32(1 << DG_OCTREE_MAX_DEPTH))
      * dgFloat32(1 << DG_OCTREE_MAX_DEPTH);
  ...
  if (!obtreeCell->m_count)
  {
    if (obtreeCell != &m_inactiveList)
    {
      dgBroadPhaseLayer::dgTreeNode* const node = m_layerMap[dgInt32(
          obtreeCell->m_layerIndex)].GetNodeFromInfo(*obtreeCell);
      _ASSERTE(node);
...
void dgBroadPhaseCalculateContactsWorkerThread::ThreadExecute()
{
  ...
  dgInt32 contactSize = dgInt32(
      m_world->m_contactBuffersSizeInBytes[m_threadIndex]
          / sizeof(dgContactPoint));
    ...
    void* const userData) const
{
  ...
  if (filter && (dist2 > dgFloat32(1.0e-8f))
      && dgRayBoxClip(ll0, ll1, m_appMinBox, m_appMaxBox))
  {
    dgLineBox line;
...
//			_ASSERTE (cell);
  ...
  if (body->m_isInWorld)
  {
    ...
    dgWorld* const me = (dgWorld*) this;
    if ((body->m_minAABB.m_x > m_appMinBox.m_x)
        && (body->m_minAABB.m_y > m_appMinBox.m_y)
        && (body->m_minAABB.m_z > m_appMinBox.m_z)
        && (body->m_maxAABB.m_x < m_appMaxBox.m_x)
        && (body->m_maxAABB.m_y < m_appMaxBox.m_y)
        && (body->m_maxAABB.m_z < m_appMaxBox.m_z))
    {
        ...
        if (ix1 == ix0)
        {
          ...
          if (iz1 == iz0)
          {
            ...
            if (newCell != cell)
            {
              ...
              {
                dgBroadPhaseLayer::dgTreeNode* const node = m_layerMap[dgInt32(
                    cell->m_layerIndex)].GetNodeFromInfo(*cell);
                _ASSERTE(node);
      ...
      if (cellsBodyCount >= dgInt32((sizeof(cellArray) / sizeof(dgCellPair))))
      {
        ...
        if (threadCounts > 1)
        {
              ...
              threadIndex++)
          {
            ...
            m_applyExtForces[threadIndex].m_count = chunkSizes[threadIndex]
                * threadCounts;
...
//	contactPair.m_count = 0;
      ...
      for (dgInt32 i = j - 1; i >= 0; i--)
      {
        ...
        if (cell1)
        {
          if (cell0->m_active | cell1->m_active)
          {
            ...
            cellsPairsCount++;
            if (cellsPairsCount
                >= dgInt32(sizeof(cellArray) / sizeof(cellArray[0])))
            {
              if (threadCounts > 1)
              {
  ...
  if (threadCounts > 1)
  {
    ...
    {
      m_cellPairsWorkerThreads[threadIndex].m_step = threadCounts;
      m_cellPairsWorkerThreads[threadIndex].m_count = chunkSizes[threadIndex]
          * threadCounts;
...
//			if (! (contact->m_body0->m_sleeping & contact->m_body1->m_sleeping) ) {
      ...
      if (!((body0->m_sleeping | body0->m_equilibrium)
          & (body1->m_sleeping | body1->m_equilibrium)))
      {
