    ...
    const dgJointCallBackParam* param) const
{
  ...
  if (m_posit > distance)
  {
    ...
    accel = dgFloat32(100.0f) * penetrationErr
        - speed * dgFloat32(1.01f) / param->m_timestep;
...
dgVector dgSlidingConstraint::GetJointForce() const
{
  ...
  return dgVector(
      matrix0.m_up.Scale(m_jointForce[0])
...
//	const dgVector& p1 = matrix1.m_posit;
  ...
  dgVector p1(
      matrix1.m_posit
          + matrix1.m_front.Scale((p0 - matrix1.m_posit) % matrix1.m_front));
  ...
  if (m_jointAccelFnt)
  {
    ...
    {
      if ((axisParam.m_minFriction > DG_MIN_BOUND)
          || (axisParam.m_maxFriction < DG_MAX_BOUND))
      {
