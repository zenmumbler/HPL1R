...
namespace hpl {
												...
												const cMatrixf &a_mtxOffset,iPhysicsWorld *apWorld)
	{
		...
		{
			iPhysicsJointSlider *pJoint = apWorld->CreateJointSlider(sNamePrefix+apMeshJoint->msName
																	,vPivot,vPinDir,apParentBody,apChildBody);
												...
												const cMatrixf &a_mtxOffset)
	{
		if(apMeshRef->msParent != "")
		{
			...
			if(pEntity) apMeshEntity->AttachEntityToParent(pEntity,apMeshRef->msParent);
			return pEntity;
		}
		else
		{
