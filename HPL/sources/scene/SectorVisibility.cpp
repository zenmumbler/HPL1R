...
namespace hpl {
	...
	bool cPortalVisibilitySet::PortalExists(cPortal *apPortal)
	{
		...
		{
			if(mvVisibility[i]->mpPortal == apPortal) return true;
		}
	...
	bool cSectorVisibility::PortalExists(cPortal *apPortal)
	{
		for(size_t i=0; i< mvVisibiltySets.size(); ++i)
		{
			if(mvVisibiltySets[i]->PortalExists(apPortal)) return true;
		}
	...
	cSectorVisibility* cSectorVisibilityContainer::GetSectorVisibilty(cSector *apSector)
	{
		...
		{
			if(mbLog) Log("%sVisibility sector for '%s' already exist!\n",GetTabs().c_str(),apSector->GetId().c_str());
			return it->second;
		}
	}
	...
	void cSectorVisibilityContainer::Compute(cPortalContainer *apContainer)
	{
		...
		if(mbLog)Log("Checking for start sectors\n");
		...
		for(; it != pSectorMap->end(); ++it)
		{
			...
			{
				if(mbLog)Log("Sector '%s' is a start!\n",pSector->GetId().c_str());
		...
		if(mbLog)Log("Done checking start sectors!\n");
	}
													...
													int alPortalIndex)
	{
		...
		for(; it != pPortalList->end(); ++it)
		{
			...
			{
				if(mbLog) Log("%sFound new valid portal %d\n",GetTabs().c_str(),pPortal->GetId());
				...
				if(pSet==NULL) {
					...
					if(mbLog) Log("%sNo portal connecting from %s to %s, creating new visibility set\n",
																		GetTabs().c_str(),
																		apSector->GetId().c_str(),
			...
			{
				if(mbLog) Log("%sSkipped unvalid portal %d\n",GetTabs().c_str(),pPortal->GetId());
