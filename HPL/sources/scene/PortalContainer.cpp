...
namespace hpl {
	...
	{
		if(mbGlobal==false && mSectorIt == mpContainer->m_mapSectors.end()) return false;
	...
	void cPortal::Compile()
	{
		...
		for(; it != mlstPoints.end(); it++)
		{
			...
			else if(vMin.y > vP.y)	vMax.y = vP.y;
			if(vMax.z < vP.z)		vMax.z = vP.z;
			else if(vMin.z > vP.z)	vMax.z = vP.z;
		}
	...
	cPortal* cSector::GetPortal(int alId)
	{
		...
		{
			cPortal *pPortal = *it;
			if(pPortal->mlId == alId) return pPortal;
		}
	...
	{
		if(gbCallbackActive==false) return;
	...
	bool cPortalContainer::Add(iRenderable *apRenderable, bool abStatic)
	{
		...
		else
		{
			...
			for(;it != m_mapSectors.end();it++)
			{
				...
				if(pSector->TryToAdd(apRenderable, false)){
					bAdded = true;
					if(bLog)Log("Adding as dynamic in sector %s\n",pSector->GetId().c_str());
				}
	...
	bool cPortalContainer::AddEntity(iEntity3D *apEntity)
	{
		...
		if(m_mapSectors.empty() == false)
		{
			if(bLog) Log(" Adding callback for %s\n",apEntity->GetName().c_str());
		...
		else
		{
			...
			for(; it != pDataList->end(); ++it)
			{
				...
				for(;it != pSector->m_setDynamicObjects.end(); ++it){
					iRenderable *pR = *it;
					if(bLog)Log("Adding dynamic '%s' type: %s\n",pR->GetName().c_str(),pR->GetEntityType().c_str());
